...
public class FaultToleranceIntegrationTests : IClassFixture<GpuClusterFixture>
{
    ...
    public async Task GrainFailure_ShouldRecoverGracefully()
    {
        ...
        var grain = grainFactory.GetGrain<IGpuBatchGrain<float[], float[]>>("failure-recovery-test");
    ...
    public async Task HighErrorRate_ShouldMaintainSystemStability()
    {
        ...
        {
            var grain = grainFactory.GetGrain<IGpuBatchGrain<float[], float[]>>($"error-test-{i}");
    ...
    public async Task MemoryPressure_ShouldHandleGracefully()
    {
        // Arrange
        var grainFactory = _fixture.Cluster.GrainFactory;
        var grain = grainFactory.GetGrain<IGpuBatchGrain<float[], float[]>>("memory-pressure-test");
    ...
    public async Task TimeoutHandling_ShouldNotBlockSystem()
    {
        ...
        var slowGrain = grainFactory.GetGrain<IGpuBatchGrain<float[], float[]>>("timeout-test-slow");
        var fastGrain = grainFactory.GetGrain<IGpuBatchGrain<float[], float[]>>("timeout-test-fast");
    ...
    public async Task ResourceExhaustion_ShouldFallbackGracefully()
    {
        ...
        {
            var grain = grainFactory.GetGrain<IGpuBatchGrain<float[], float[]>>($"exhaustion-test-{i}");
            grains.Add(grain);
    ...
    public async Task PartialFailure_BatchProcessing_ShouldContinueProcessing()
    {
        // Arrange
        var grainFactory = _fixture.Cluster.GrainFactory;
        var grain = grainFactory.GetGrain<IGpuBatchGrain<float[], float[]>>("partial-failure-test");
    ...
    public async Task SystemRecovery_AfterMultipleFailures_ShouldStabilize()
    {
        ...
        for (int phase = 0; phase < recoveryPhases; phase++)
        {
            ...
            {
                var grain = grainFactory.GetGrain<IGpuBatchGrain<float[], float[]>>($"recovery-{phase}-{op}");
